...
<link rel="stylesheet" href="easyui/jquery.min.js">
...
/* 
 ...
 <script type="text/javascript">
	//更新验证码
	function updateyzm(){
		$("#veryCode").prop("src","YZM?"+Math.random());
	}
 </script>
</head>
    ...
    <div class="cont_login">
      <div class="cont_forms cont_forms_active_login" style="height: 480px;width: 400px;border-radius: 15px;">
       <!-- 	<div class="cont_img_back_"> <img src="images/timg.jpeg" alt="背景图片" /> </div> -->
        <div class="cont_img_back_"> <img src="images/beijing2.jpg" alt="背景图片" /> </div>
        <form action="AoaUser/Login" method="post" >
        <div class="cont_form_login" style="display: block;opacity: 1;width: 400px;">
          ...
          <h2>登录</h2>
          <input type="text" placeholder="用户名" autofocus="autofocus" value="" name="userName" class="userName test"/>
          <input type="password" placeholder="密码" value="" name="password" class="password test"/>
           ...
           <div class="login-img" style="display:inline-block">
					<img id="veryCode" class="thisimg" onclick="updateyzm()" src="YZM" alt="验证码" style="width: 100px;height: 42px;border-radius: 3px;">
				</div>
		    <br>
       <button class="btn_login btn" type="button"  onclick="return check()">登录</button>
        </div>
        </form>
      </div>
     <div style="border: 1px solid transparent;">
      	<p style="margin-top: 550px;font-weight: 400;color: #757575;font-size: 22px;letter-spacing:1px ;">欢迎使用华瑞OA办公自动化系统</p>
      </div>
    </div>
...
function alertError(className,loc){
	$('.error-mess').text(loc);
	$('.alert-danger').css('display','block');
	$(className).css('border-color',"#a94442");
}
...
function check() {
	...
	if(userName==null || userName==""){
		alertError(".userName","登录用户名不能为空!");
		count=0;
		return false;
	}
	if(password==null || password==""){
		alertError(".password","登录密码不能为空!");
		count=0;
		return false;
	}
	if(code==null || code==""){
		alertError(".code","验证码不能为空!");
		count=0;
		return false;
	}
	$.post("${pageContext.request.contextPath}/AoaUser/Login",
        	 {
					"yzm":code,
					"userName":userName,
					"pwd":password,
             ...
             function(data){
			 		if(data=="no"){
			 			alertError(".code","验证码错误!");
			 			//刷新验证码
						updateyzm();
						$('.code').val("");
					} else{
						if(data.trim()=="lgoinYes"){
							//location.href="${pageContext.request.contextPath}/AoaSysMenu/queryIndex";
							location.href="index.jsp";
						}else{
							if(data.trim()=='Error'){
					 			alertError(".userName","用户名或密码错误!");
							}
						}
					} 
			});
