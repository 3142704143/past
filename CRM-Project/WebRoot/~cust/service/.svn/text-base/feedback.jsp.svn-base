...
<div class="button_bar">
	<button class="common_button" onclick="help('暂无此方面帮助');">帮助</button>
	<button class="common_button" onclick="document.getElementById('form1').submit();">查询</button>  
</div>
<form action="${pageContext.request.contextPath}/cs!csList?c=1" method="post" id="form1">
	...
	<s:iterator value="#request.list" var="c">
		<tr>
			...
			<td class="list_data_op">
				<img onclick="to('cs!queryCs?cs.svrId=${c.svrId}&c=1');" title="处理" src="${pageContext.request.contextPath}/images/bt_feedback.gif" class="op_button" />
			</td>
		</tr>
	</s:iterator>
	<tr>
			...
			<div class="pager">
				<s:if test="#request.csSum!=0">
					...
					<s:if test="#request.cpage==1 and #request.csCount>1">
						<a href="cs!csList?c=1&cpage=${cpage+1}&count=${count}">下一页</a>
						<a href="cs!csList?c=1&cpage=${csCount}&count=${count}">最后一页</a>
					</s:if>
					<s:elseif test="#request.ccsount>#request.cpage and #request.cpage!=1">
						<a href="cs!csList?c=1&cpage=1&count=${count}">第一页</a>
						<a href="cs!csList?c=1&cpage=${cpage-1}&count=${count}">上一页</a>
						<a href="cs!csList?c=1&cpage=${cpage+1}&count=${count}">下一页</a>
						<a href="cs!csList?c=1&cpage=${csCount}&count=${count}">最后一页</a>
					</s:elseif>
					...
					<button type="button" onclick="subGo(${allSum},'cs!csList?c=1')">GO</button>
				</s:if>
				<s:else>
					无任何记录
				</s:else>
