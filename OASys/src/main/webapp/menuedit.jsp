...
<div class="row" style="padding-top: 10px;">
	...
	<div class="col-md-10 text-right">
		<a href="${pageContext.request.contextPath }/AoaSysMenu/SelectAll?name="><span class="glyphicon glyphicon-home"></span> 首页</a> > <a
...
<div class="row" style="padding-top: 15px;">
	<div class="col-md-12">
		...
		<div class="bgc-w box">
			...
			<form action="${pageContext.request.contextPath}/AoaSysMenu/toUpdate" method="post" id="thisForm" onsubmit="return check();">
					...
					<div class="box-body">
						...
						<div class="row">
							<div class="col-md-6 form-group">
								...
								<input class="form-control" value="${menu.menuName }" name="menuName"/>
							</div>
							<div class="col-md-6 form-group">
								...
								<input class="form-control" value="${menu.menuIcon }"  name="menuIcon"/>
							</div>
							<div class="col-md-6 form-group">
								<label class="control-label"><span>路径</span></label>
								 <input	class="form-control" value="${menu.menuUrl }" name="menuUrl"/>
							</div>
							<div class="col-md-6 form-group">
								<label class="control-label"><span>排序</span></label>
								 <input type="number" min="0" class="form-control" value="${menu.sortId }" name="sortId"/>
							</div>
							<div class="col-md-6 form-group">
								<label class="control-label"><span>父级</span></label>
								<c:if test="${menu.parentId!=0 }">
									<select class="form-control" name="parentId">
									 	<c:forEach items="${parentMenu }" var="r">
									 		<option value="${r.menuId }" ${r.menuId==menu.parentId ? 'selected' :'' }>${r.menuName }</option>
									 	</c:forEach>
								 	</select>
								</c:if> 
								 <c:if test="${menu.parentId==0 }">
								 	<input type="number" min="0" max="0" class="form-control" value="0"  name="parentId"/>
								 </c:if>
							</div>
							<div class="col-md-6 form-group">
								<label class="control-label"><span>显示</span></label><br>
								<span class="labels"><label><input id="ck" onchange="show();" value="${menu.isShow }" name="isShow" type="checkbox"  ${menu.isShow==1?"checked":""}><i>✓</i></label></span>
							</div>
						</div>
				</div>
				</div>
