...
<body style="background-color: #ecf0f5;">
	...
	<div class="row" style="padding-top: 15px;">
		<div class="col-md-12">
			<!--id="container"-->
			<div class="bgc-w box">
				<form action="${pageContext.request.contextPath }/AoaPosition/Update" method="post" onsubmit="return check();">
					<!--盒子头-->
							...
							<div class="row">
								<div class="col-md-6 form-group">
									<label class="control-label"><span>名称</span></label> <input
										name="name" class="form-control" value="${aoaPosition.name }"/>
								</div>
								<div class="col-md-6 form-group">
									<label class="control-label"><span>层级</span></label> <input
										name="level" class="form-control" value="${aoaPosition.level }"/>
								</div>
								<div class="col-md-6 form-group">
									<label class="control-label"><span>部门</span></label> 
									<select class="deptselect form-control" name="deptid">
											<c:forEach items="${list }" var="v">
											<option ${aoaPosition.deptid==1?"selected":"" } value="${v.deptId }">${v.deptName }</option>
											</c:forEach>
									</select>
								</div>
								<div class="col-md-6 form-group">
									<label class="control-label"><span>描述</span></label>
									<textarea rows="5" cols="20" name="describtion"  class="form-control" style="margin: 0px -0.5px 0px 0px; height: 114px; width: 507px;">${aoaPosition.describtion }</textarea>
								</div>
								<input type="hidden" name="positionId" value="${aoaPosition.positionId }"/>
							</div>
						</div>
					</div>
					<!--盒子尾-->
