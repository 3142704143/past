...
<body style="background-color: #ecf0f5;">
					...
					<div class="box-body no-padding">
						<div class="box-body">
							<div class="row">
								...
								<div class="col-md-12 form-group">
									部门经理：${aoaUser.realName }
									
										<a class="changemanage label xiugai" onclick ="dianji1('${aoaUser.deptId}','${aoaUser.positionId}');">更换</a>
								</div>
								...
								<table class="table table-hover table-striped">
									...
									<c:forEach items="${list }" var="v">
										<tr>
											<td><img src="${pageContext.request.contextPath }/images/timg.jpg" class="img-circle"
												style="width: 25px; height: 25px;" /></td>
											<td><span>${v.realName }</span></td>
											<td><span>${v.userName }</span></td>
											<td><span>${v.name }</span></td>
											<td>
												<a id="${v.userId }" class="usermanagechange label xiugai" onclick="dianji('${v.deptId}','${v.positionId }');">
													<span class="glyphicon glyphicon-edit"></span> 人事调动
												</a>
												<input class="userid" type="hidden" value="${v.userId }"/>
												<input class="realname" type="hidden" value="${v.realName }"/>
											</td>
										</tr>
									</c:forEach>	
								</table>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body box no-padding" style="display: none;">
					<form action="${pageContext.request.contextPath}/AoaDept/PersonnolTransfer" method="post">
						<div class="box-header">
						...
						<div class="box-body no-padding">
							<div class="row">
								...
								<div class="col-md-6">
									...
									<select class="deptselect form-control" name="deptId" id="bumen">
								...
								<div class="col-md-6">
									...
									<select class="positionselect form-control" name="positionId" id="zhiwei">
										<c:forEach items="${apsList}" var="v">
											<option value="${v.positionId }">${v.name }</option>
										</c:forEach>
									</select>
								</div>
							</div>
	...
	<div class="modal fade in managechangemodel" id="thismodal" data-backdrop="static">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body box no-padding" style="display: none;">
					<form action="${pageContext.request.contextPath}/AoaDept/PersonnolTransfer2" method="post">
						<div class="box-header">
							<h3 class="box-title mc-title" style="font-size:15px;">人事变动</h3>
						</div>
						<div class="box-body no-padding">
							<div class="row">
									...
									<div class="col-md-6">
										<label class="control-label"> <span>部门</span></label>
										<input type="hidden" value="${aoaUser.userId }" name="userId1">
										<select class="deptselect form-control" name="deptId1" id="bumen1">
											<c:forEach items="${adsList}" var="v">
												<option  value="${v.deptId }">${v.deptName }</option>
											</c:forEach>
										</select>
									...
									<div class="col-md-6">
										<label class="control-label"> <span>职位</span></label>
										<select class="positionselect form-control" name="bbb" id="zhiwei1">
											<c:forEach items="${apsList}" var="v">
												<option  value="${v.positionId }">${v.name }</option>
											</c:forEach>
										</select>
									...
									<div class="col-md-6">
											...
											<input type="hidden" name="aaa"  value="${aoaUser.deptId }">
...
$(".deptselect").on("change",function(){
	...
	function  dianji(deptId,positionId){
...
/* $(".changemanage").click(function(){
	...
	function dianji1(deptId,positionId){
		$("#thismodal.managechangemodel").modal("toggle");
		$('#thismodal.managechangemodel .modal-body').css('display', 'block');
		$("#bumen1").val(deptId);
		$("#zhiwei1").val(positionId);
	}
