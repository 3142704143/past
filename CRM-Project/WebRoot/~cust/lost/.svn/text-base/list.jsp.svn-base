...
<form action="cst!cstList?a=1" method="post">
...
<table class="data_list_table">
	...
	<c:forEach items="${list}" var="c">
		...
		<td>${c.lstLostDate}</td>
		<td>
			<c:if test="${c.lstStatus==1}">警告</c:if>
			<c:if test="${c.lstStatus==2}">暂缓流失</c:if>
			<c:if test="${c.lstStatus==3}">已经流失</c:if>
		</td>
		<td>
				<img onclick="to('cst!queryCst?cl.lstId=${c.lstId}');" title="确认流失" src="${pageContext.request.contextPath}/images/bt_confirm.gif" class="op_button" />
				<img onclick="to('cst!queryCst?cl.lstId=${c.lstId}&a=1');" title="暂缓流失" src="${pageContext.request.contextPath}/images/bt_relay.gif" class="op_button" />
			...
			<div class="pager">
				<s:if test="#request.cstSum!=0">
					...
					<s:if test="#request.cpage==1 and #request.cstCount>1">
						<a href="cst!cstList?a=1&cpage=${cpage+1}&count=${count}">下一页</a>
						<a href="cst!cstList?a=1&cpage=${cstCount}&count=${count}">最后一页</a>
					</s:if>
					<s:elseif test="#request.cstCount>#request.cpage and #request.cpage!=1">
						<a href="cst!cstList?a=1&cpage=1&count=${count}">第一页</a>
						<a href="cst!cstList?a=1&cpage=${cpage-1}&count=${count}">上一页</a>
						<a href="cst!cstList?a=1&cpage=${cpage+1}&count=${count}">下一页</a>
						<a href="cst!cstList?a=1&cpage=${cstCount}&count=${count}">最后一页</a>
					</s:elseif>
					<s:elseif test="#request.cstCount==#request.cpage and #request.cpage!=1">
					...
					<button type="button" onclick="subGo(${allSum},'cst!cstList?a=1')">GO</button>
				</s:if>
				<s:else>
					无任何记录
				</s:else>
